...
public class PermisoDAOimpl extends ConexionDB implements PermisoDAO{
    ...
    public List<Permiso> getAll() throws Exception {
        ...
        try {
            this.conectar();
            PreparedStatement ps=this.conn.prepareStatement("SELECT p.*, u.usuario as 'usuario', r.descripcion as 'rol' FROM usuarios u, roles r, permisos p where p.id_usuario=u.id AND p.id_rol=r.id;");
            ResultSet rs = ps.executeQuery();
